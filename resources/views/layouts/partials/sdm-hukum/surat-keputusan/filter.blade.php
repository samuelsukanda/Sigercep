<div class="bg-white rounded-xl shadow-sm border border-gray-200 mb-4">
    <div class="px-5 py-4">
        <form method="GET" action="{{ route('sdm-hukum.surat-keputusan.index') }}" id="filterForm">
            <div class="flex flex-wrap gap-3 items-end filter-wrap">

                @include('layouts.partials.filter-periode')
                {{-- Unit --}}
                @php
                    $units = config('units.sk', []);
                @endphp
                ...
                <div class="flex items-end flex-1 justify-between filter-action">
                    ...
                    {{-- Tambah Data --}}
                    @canAccess('surat_keputusan', 'create')
                    <a href="{{ route('sdm-hukum.surat-keputusan.create') }}"
