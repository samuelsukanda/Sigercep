...
let table = $("#kesiapanAmbulanceTable").DataTable({
    ...
    columns: [
        {
            data: "mobil_ambulance",
            render: function (data) {
                return `<div class="flex items-center gap-2"><i class="fas fa-ambulance text-blue-500 text-lg"></i> ${data}</div>`;
            },
        },
        {
        ...
        },
        {
            ...
            render: function (data) {
                ...
                if (data.can_update) {
                    btn += `
                    <a href="/kesiapan-ambulance/${data.id}/edit"
                ...
                if (data.can_read) {
                    btn += `
                        <a href="/kesiapan-ambulance/${data.id}"
                ...
                if (data.can_delete) {
                    btn += `
                            <form action="/kesiapan-ambulance/${data.id}" method="POST" style="display:inline;">
