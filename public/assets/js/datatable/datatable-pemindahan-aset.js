...
let table = $("#pemindahanAsetTable").DataTable({
    ...
    columns: [
        ...
        },
        {
            data: "unit_asal",
            render: function (data) {
                return `<span class="px-2 py-1 text-xs font-semibold rounded-full bg-orange-100 text-orange-800">${data}</span>`;
            },
        },
        {
            data: "unit_tujuan",
        ...
        },
        {
            ...
            render: function (data) {
                ...
                if (data.can_update) {
                    btn += `
                    <a href="/pengadaan-aset/pemindahan-aset/${data.id}/edit"
                ...
                if (data.can_read) {
                    btn += `
                        <a href="/pengadaan-aset/pemindahan-aset/${data.id}"
                ...
                if (data.can_delete) {
                    btn += `
                            <form action="/pengadaan-aset/pemindahan-aset/${data.id}" method="POST" style="display:inline;">
